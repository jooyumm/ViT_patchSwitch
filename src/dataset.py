...
# 우리가 쓰는 체크포인트(vit_base_patch{8,16}_224 augreg 계열, 이 프로젝트는 P8/P16만
# 다룸)는 학습 시 표준 ImageNet 정규화가 아니라 mean=std=0.5(inception 스타일)를 썼다.
# timm의 pretrained_cfg에서 직접 읽어와서 하드코딩 오차를 없앤다.
# (원 프로젝트 ViT_tradeoff에서 P=8/16/32 세 체크포인트 모두 동일한 설정을 공유함을 확인함)
