...
1) **기저 비용(baseline)**: 모든 이미지에 항상 발생 — P16 예측 + 탐지(+ local_switch에 필요한
   위치특정까지) 를 **한 번의 forward**로 계산(defense.detector.predict_detect_localize).
...
from defense.detector import predict_detect_localize
...
def main():
    ...
    sys_cand = glob.glob(os.path.join(HERE.replace('/experiments/cost_comparison', '/results/system_comparison'),
