...
하나의 표 + 하나의 그림으로 합친다. 새 GPU 실험이 아니라 기존 npz 5개를 읽어서 계산만 한다:
  ...
  - results/system_comparison/system_comparison_n*.npz   (복원율/시스템 정확도/clean 비용)
  - results/cost_comparison/cost_comparison.npz            (파이프라인 비용)
  - results/all_switch/joint_attack/07_joint_attack_test_n50.npz            (naive joint attack)
  - results/local_switch/joint_attack/17_local_swap_joint_attack_n50.npz     (naive joint attack)
  - results/all_switch/adaptive_evasion_full/14_..._n50_seed123.npz          (완전판 adaptive evasion)
  - results/local_switch/adaptive_evasion_full/18_..._n50_seed123.npz        (완전판 adaptive evasion)
...
def main():
    sys_cand = glob.glob(os.path.join(ROOT, 'results/system_comparison/system_comparison_n*.npz'))
    cost_cand = glob.glob(os.path.join(ROOT, 'results/cost_comparison/cost_comparison.npz'))
    assert sys_cand, "results/system_comparison/에 npz 없음 — system_comparison_test.py 먼저 실행"
    assert cost_cand, "results/cost_comparison/에 npz 없음 — cost_comparison_test.py 먼저 실행"
    sysd = np.load(max(sys_cand, key=os.path.getmtime))
    costd = np.load(cost_cand[0])

    n_all_joint, n_all_base = joint_defeat_from_raw(
        os.path.join(ROOT, 'results/all_switch/joint_attack/07_joint_attack_test_n50.npz'))
    all_joint_rate = n_all_joint / n_all_base
    all_joint_ci = wilson_ci(n_all_joint, n_all_base)

    loc_joint_d = np.load(os.path.join(
        ROOT, 'results/local_switch/joint_attack/17_local_swap_joint_attack_n50.npz'))
    ...
    all_evasion_cand = glob.glob(os.path.join(
        ROOT, 'results/all_switch/adaptive_evasion_full/14_*_n50_seed123.npz'))
    loc_evasion_cand = glob.glob(os.path.join(
        ROOT, 'results/local_switch/adaptive_evasion_full/18_*_n50_seed123.npz'))
