"""
probes/vitguard_layer_sweep.py — [탐색적 검증, 롤백 가능] 레이어별 · 방식별(rollout vs raw attention)
집중도 신호와 AUROC 비교.

배경
----
vitguard_signature.py(12레이어 전체 rollout)의 결과가 애매했다 — PatchFool의 top-4 질량비율이
...
- clean / LaVAN / PatchFool 세 조건 각각, 모델 forward 1회로 12개 레이어의 attention을 전부 수집
  (attack 생성은 재사용 불가 — 재실행하되 seed 고정이라 vitguard_signature.py와 같은 샘플/공격이 나옴)
...
주의: 원본 vitguard_signature.py와 src/, experiments/ 코드는 전혀 건드리지 않음.
     이 파일도 필요없어지면 지우기만 하면 됨.

사용법:
  python probes/vitguard_layer_sweep.py --patch_size 16 --num_samples 30 --seed 42
"""
import argparse
import os
import sys
...
def main():
    ...
    # 동일한 고정 이미지 집합으로 비교됨 (ViT_patchSwitch/README.md "샘플링 감사" 참고)
