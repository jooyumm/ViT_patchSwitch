"""
probes/vitguard_range_evasion.py — [탐색적 검증, 롤백 가능] 지난 실패 4개 샘플(1,15,23,28)의
실제 saliency 값 범위를 실측하고, 그 구간을 의도적으로 노린 공격이 탐지기를 뚫는지 확인.

배경
----
vitguard_evasion_test.py에서 "saliency 최저" 강제는 오히려 탐지가 잘 됐다(recall@1=0.90) —
근데 실패했던 4개 샘플(attention 기반 선택, vitguard_failure_case.py)은 saliency 최저가
...
방법
----
1) attn_layer_idx=4 attention 기반 선택으로 정답 토큰을 재현(vitguard_failure_case.py와 동일
...
사용법:
  python probes/vitguard_range_evasion.py --patch_size 16 --num_samples 30 --seed 42
"""
import argparse
import os
import sys
...
KNOWN_FAILURE_SAMPLES = [1, 15, 23, 28]  # vitguard_failure_case.py에서 확인된 실패 인덱스
...
def main():
    ...
    # 1) attention 기반 선택으로 정답 토큰 재현 (vitguard_failure_case.py와 동일 설정)
    ...
    out_dir = os.path.dirname(os.path.abspath(__file__)).replace('/experiments/', '/results/', 1)
