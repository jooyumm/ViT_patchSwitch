"""
probes/vitguard_evasion_test.py — [탐색적 검증, 롤백 가능] PatchFool을 "이미지 saliency 최저
영역"으로 강제했을 때, L=12 raw-attention 탐지기를 일관되게 회피하는지 확인. 회피가 확인되면
attention을 이미지 saliency로 정규화한 점수가 이 약점을 줄이는지도 같이 본다.

배경
----
vitguard_failure_case.py에서 발견한 4개 실패 샘플이 전부 "공격이 저saliency 배경에 떨어지고,
...
사용법:
  python probes/vitguard_evasion_test.py --patch_size 16 --num_samples 30 --seed 42
"""
import argparse
import os
import sys
...
def main():
    ...
    print(f"(참고: attention 기반 위치 선택 baseline은 30/30 전부 라벨 변경됨 — vitguard_failure_case.py 로그)")
