...
5) 공유 탐지기(defense.detector)로 L=12 top4_mass 점수를 계산하고, calibration에서만
   Youden 임계값을 정한다(순환평가 방지, §6와 동일 원칙).
...
주의: src/attacks/patch_fool.py는 import만(수정 없음). defense/ 세 모듈도 import만.
...
from defense.detector import collect_layer_attn, raw_at_layer, top4_mass, localize_top1
