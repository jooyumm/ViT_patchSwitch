"""
probes/vitguard_adaptive_evasion_full_test.py — [탐색적 검증, 롤백 가능] 완전판 adaptive
attack(P16+P8 동시 공격 + 탐지 회피 제약) stress test. 지금 방어의 진짜 worst-case.

배경
----
§7(vitguard_joint_attack_test.py)의 adaptive attacker는 방어 구조(P16+P8)는 알지만 탐지기
...
이미지 집합은 §7(vitguard_joint_attack_test.py)과 동일(seed=123, n=50, eval_utils의
...
사용법:
  python probes/vitguard_adaptive_evasion_full_test.py --seed 123 --num_samples 50 --chunk 20
"""
import argparse
import os
import sys
...
# §7(vitguard_joint_attack_test.py, 같은 seed=123/n=50)의 참고값
