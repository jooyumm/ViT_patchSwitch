"""
"의심되면 P16→P8 전체 재분류" 방어의 핵심 검증

배경
----
"의심 영역만 P8로 국소 재분할"은 P8/P16이 완전히 독립적으로 학습된 별개 체크포인트라서
(임베딩 공간이 안 섞임) 지금은 불가능함을 확인했다. 

대신 같은 핵심 아이디어("P16이 못 미더우면 상대적으로 강건한 P8을 대신 쓴다")를 "의심되면 이미지 전체를 P8로 통째로 재분류"로 단순화해서
검증한다.
...
def main():
    ...
    # [재구성 주의] 01_layer_sweep_P16_raw.npz는 ViT_patchSwitch/results/01_detection_localization/01_signature/에
    # 있음(defense/01_detection_localization/01_signature/vitguard_layer_sweep.py 산출물) --
    # 이 스크립트(vitguard_p8_rescue_test.py)는 archive/06_p8_rescue_test/에 있지만 원래
    # 01_detection_feasibility의 산출물을 재사용하던 관계라 경로가 폴더 경계를 넘어감
    # (ViT_robust/probes/ 시절부터 있던 관계, 원래도 그랬음)
